      ...
      <div className="mt-6 grid gap-3">
        {filtered.map((item, index) => (
          <button key={item} onClick={() => setOpen(index)} className="rounded-3xl bg-white p-5 text-left dark:bg-white/5">
            ...
            {open === index ? <p className="mt-3 leading-7 text-neutral-500 dark:text-neutral-400">Velora policies are designed to be simple, fast, and transparent across every order.</p> : null}
          </button>
        ))}
      </div>
    </section>

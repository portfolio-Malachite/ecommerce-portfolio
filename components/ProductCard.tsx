  ...
  return (
    ...
    <motion.article whileHover={{ y: -6 }} className="group overflow-hidden rounded-3xl border border-black/5 bg-white shadow-sm transition dark:border-white/10 dark:bg-white/5">
      <Link href={`/product/${product.slug}`} className="relative block aspect-[4/5] overflow-hidden bg-neutral-100">
        <Image src={product.image} alt={product.name} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover transition duration-700 group-hover:scale-105" />
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <button onClick={() => toggleWishlist(product.id)} className="focus-ring rounded-full p-2 hover:bg-neutral-100 dark:hover:bg-white/10" aria-label="Toggle wishlist">
            <Heart size={19} className={saved ? "fill-accent text-accent" : ""} />
          </button>
        </div>
